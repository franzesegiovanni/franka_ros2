...
CartesianImpedanceExampleController::CallbackReturn CartesianImpedanceExampleController::on_activate(
  ...
  Eigen::Map<Eigen::Matrix<double, 7, 1>> tau_initial(
      init_robot_state_.measured_joint_state.effort.data());
...
controller_interface::return_type CartesianImpedanceExampleController::update(
    const rclcpp::Time&,      // NOLINT(readability-identifier-naming)
    const rclcpp::Duration&   // NOLINT(readability-identifier-naming)
) {
  ...
  // RCLCPP_INFO(get_node()->get_logger(), "Current position: [%.4f, %.4f, %.4f]", position.x(), position.y(), position.z());
  // orientation error
  // RCLCPP_INFO(get_node()->get_logger(), "Position error: [%.4f, %.4f, %.4f]", position_error(0), position_error(1), position_error(2));
  ...
  position_error.tail(3) << error_quaternion_angle_axis.axis() * error_quaternion_angle_axis.angle();
  ...
  // RCLCPP_INFO(get_node()->get_logger(), "Cartesian stiffness:\n%s", 
  //   ((std::ostringstream&)(std::ostringstream() << cartesian_stiffness_)).str().c_str());
  ...
  // Desired torque
  // RCLCPP_INFO(get_node()->get_logger(), "tau_task: [%s]", 
  //   ((std::ostringstream&)(std::ostringstream() << tau_task.transpose())).str().c_str());
  ...
  tau_j_d_ = tau_d;
  // RCLCPP_INFO(get_node()->get_logger(), "tau_d: [%s]", 
  //   ((std::ostringstream&)(std::ostringstream() << tau_d.transpose())).str().c_str());
